...
!   NDF    - degree of freedom 
  READ (IIN,'(A80,/,5I5)') HED,NUMNP,NUMEG,NLCASE,MODEX,NDF      ! 1&2
  ...
  WRITE (IOUT,"(/,' ',A80,//,  &
     ...
     '         EQ.0, DATA CHECK',/,                                 &
      '         EQ.1, EXECUTION',/,                                   &
      '    NUMER OF ELEMENT POINTs DEGREE OF FREEDOM',5('.'),'(NDF)  =',I5 )") HED,NUMNP,NUMEG,NLCASE,MODEX,NDF
...
!   Z(NUMNP)    : Z coordinates
  ...
  CALL MEMALLOC(1,"ID   ",NDF*NUMNP,1)
  CALL MEMALLOC(2,"X    ",NUMNP,ITWO)
  CALL MEMALLOC(3,"Y    ",NUMNP,ITWO)
  CALL MEMALLOC(4,"Z    ",NUMNP,ITWO)

  CALL INPUT (IA(NP(1)),DA(NP(2)),DA(NP(3)),DA(NP(4)),NUMNP,NEQ)      !      INPUT (ID,X,Y,Z,NUMNP,NEQ)
...
!       FLOAD(NLOAD) : Magnitude of load
     ...
     CALL LOADS (DA(NP(5)),IA(NP(6)),IA(NP(7)),DA(NP(8)),IA(NP(1)),NLOAD,NEQ)       !NP为矢量指针 LOADS (R,NOD,IDIRN,FLOAD,ID,NLOAD,NEQ)
...
!    Triangularize stiffness matrix
  ...
  IND=3    ! Stress calculations
     ...
     REWIND ILOAD
     DO L=1,NLCASE
        CALL LOADV (DA(NP(4)),DA(NP(6)),NEQ)   ! Read in the load vector
