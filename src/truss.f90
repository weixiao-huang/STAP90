...
SUBROUTINE RUSS (ID,X,Y,Z,U,MHT,E,AREA,DENSITY,LM,XYZ,MATP)
! . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
! .                                                                   .
...
! . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
  ...
  REAL(8) :: X(NUMNP),Y(NUMNP),Z(NUMNP),E(NPAR(3)),AREA(NPAR(3)),  &
            DENSITY(NPAR(3)), XYZ(6,NPAR(2)),U(NEQ)                             ! 在材料属性中添加了密度参数
...
! Read and generate element information
  IF (IND .EQ. 1) THEN
     ...
     DO I=1,NUMMAT
        READ (IIN,'(I5,3F10.0)') N,E(N),AREA(N),DENSITY(N)                      ! Read material information  添加密度
        WRITE (IOUT,"(I5,4X,E12.5,2X,E14.6,2X,E12.5)") N,E(N),AREA(N),DENSITY(N)
     END DO
...
! Assemble stucture stiffness matrix
  ELSE IF (IND .EQ. 2) THEN
     ...
     DO N=1,NUME
        ...
        XM=DENSITY(MTYPE)*AREA(MTYPE)*XL        !rou*A*l
        ...
        CALL ADDgravity(DA(NP(6)),XM,LM(1,N),ND)                              !6为新开的存储重力的数组   可以在这里组装重力
     END DO
